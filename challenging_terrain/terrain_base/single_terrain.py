...
class single_terrain:
    ...
    def stair(terrain,
        ...
        step_height = round((height_range[0])/terrain.vertical_scale)
        step_x = round((size_range[1])/terrain.horizontal_scale)
        total_step_height = 0

        dis_x = start_x + platform_size

        for i in range(num_goals):
            goals[i]=[dis_x+step_x*i,start_y+per_y]
        ...
        terrain.height_field_raw[dis_x:start_x+step_x*num_goals+2*platform_size,start_y:start_y + per_y*2] = total_step_height

        return terrain,goals,start_x+step_x*num_goals
